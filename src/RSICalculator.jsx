...
export default function RSICalculator({ activePlayer, selectedPlayerId, onSaveSuccess, language = 'ar', playerHistory }) {
  ...
  const getRSIEval = (rsi) => {
    ...
    if (score >= 1.5 && score < 2.0)
      return { text: 'مقبول ⚡ (صلابة متوسطة، تحتاج لدمج قفزات Pogo قصيرة)', color: 'text-blue-400 bg-blue-950/20 border-blue-900/30' };
    if (score >= 2.0 && score < 2.5)
      return { text: 'جيد ⭐ (صلابة كاحل ممتازة وكفاءة ارتداد قوية)', color: 'text-cyan-400 bg-teal-950/20 border-teal-800/30' };
    return { text: 'نخبة أولمبية 👑 (صلابة أوتار استثنائية وسرعة نقل طاقة فائقة)', color: 'text-blue-400 bg-blue-950/20 border-blue-800/30' };
  };
          ...
          <div className="flex flex-col items-center w-full relative">
            ...
            <div className="w-full bg-[#0a1224]/50 p-4 rounded-2xl border border-gray-850 mb-5">
              <div className="flex items-center justify-between text-[10px] text-gray-400 mb-2 font-bold px-1">
                ...
                <span className="text-blue-400">اسحب خط الزمن لتحديد لحظات الكبح والارتقاء ⏱️</span>
              </div>
              ...
              <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 relative mb-4">
                ...
                <div className="flex justify-between items-center w-full md:w-auto bg-slate-900/60 px-3 py-2 rounded-xl border border-gray-850 text-center shrink-0 gap-3">
                  <div className="text-right">
                    ...
                    <span ref={currentTimeDesktopRef} className="text-xs font-black text-blue-400 font-mono">{currentTime.toFixed(3)}s</span>
                  </div>
                ...
                <div ref={timelineTrackRef} className="relative w-full md:flex-1 h-12 bg-[#080b12] border border-blue-500/30 rounded-2xl select-none flex items-center px-4">
                  ...
                  <div className="absolute inset-0 flex justify-between items-center px-4 pointer-events-none opacity-20">
                    {Array.from({ length: 25 }).map((_, i) => (
                      <div key={i} className={`w-0.5 bg-blue-500/80 rounded-full ${i % 5 === 0 ? 'h-5 opacity-70' : 'h-2.5 opacity-30'}`} />
                    ))}
                  </div>

                  {/* Highlights range between Touchdown and Landing */}
                  {duration > 0 && touchdownTime > 0 && landingTime > touchdownTime && (
                    <div className="absolute top-2.5 bottom-2.5 bg-blue-500/10 border-l border-r border-blue-500/20 pointer-events-none" style={{ right: `${(touchdownTime / duration) * 100}%`, left: `${100 - (landingTime / duration) * 100}%` }} />
                  )}
      ...
      <div className="glass-card p-5 mb-6 space-y-4 text-xs font-bold">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-4">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors duration-200 outline-none cursor-pointer ${
                    fpsAutoDetectEnabled ? 'bg-blue-500' : 'bg-gray-700'
                  }`}
                >
                  <span
          ...
          <div>
            ...
            <div className="glass-input w-full p-2.5 text-cyan-400 font-mono font-bold text-center border-cyan-500/20">{takeoffTime.toFixed(3)}</div>
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-blue-500/30 pt-4" style={{ direction: 'rtl' }}>
          <div className="space-y-3">
            <div>
              <label className="block text-[10px] text-gray-500 mb-1">طريقة حساب الوقت (Time Calculation)</label>
              <div className="flex bg-black/25 rounded-xl border border-blue-500/30 p-1">
                {[
                  ...
                  <button
                    ...
                    className={`flex-1 py-1.5 px-1 text-[10px] font-black rounded-lg transition-all cursor-pointer ${timeCalculationMethod === method.id ? 'bg-blue-500/20 text-blue-400 border border-blue-500/25' : 'text-gray-400 hover:text-white bg-transparent border border-transparent'}`}
                  >
                    {method.name}
                  </button>
                ))}
          ...
          {timeCalculationMethod === 'manual' && (
            <div className="bg-blue-950/10 border border-blue-500/25 p-3 rounded-xl animate-fade-in space-y-1.5 self-start">
              <div className="flex justify-between items-center text-[10px] text-gray-400">
                ...
                <button
                  ...
                  className="text-[8px] text-blue-400 font-bold bg-blue-950/50 border border-blue-500/20 px-2 py-0.5 rounded hover:bg-blue-900/30 cursor-pointer"
                >
        ...
        <div className="space-y-6 border-t border-gray-800/80 pt-6 animate-fade-in">
          ...
          <div className="flex justify-between items-center border-b border-blue-500/20 pb-3">
            ...
            <div className="flex items-center gap-1.5 bg-black/45 p-1 rounded-xl border border-gray-850">
              <button
                ...
                className={`px-3 py-1 text-[10px] font-bold rounded-lg transition-all cursor-pointer ${displayUnit === 'cm' ? 'bg-blue-500/25 text-blue-400 border border-blue-500/20' : 'text-gray-400 hover:text-white bg-transparent border border-transparent'}`}
              >
                سم (Cm)
              </button>
              <button
                ...
                className={`px-3 py-1 text-[10px] font-bold rounded-lg transition-all cursor-pointer ${displayUnit === 'in' ? 'bg-blue-500/25 text-blue-400 border border-blue-500/20' : 'text-gray-400 hover:text-white bg-transparent border border-transparent'}`}
              >
                بوصة (Inches)
              </button>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full">
            ...
            <button
              ...
              className="flex-1 py-4 bg-blue-600/30 border border-blue-500/50 hover:bg-blue-600/50 text-blue-400 rounded-xl font-black text-base shadow-lg transition-transform hover:scale-[1.01] active:scale-98 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              <Video size={16} />
